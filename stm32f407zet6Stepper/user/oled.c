...
{
	uint32_t IntNum;		// 整数部分
	uint32_t FraNum;		// 小数部分
	uint8_t IsNegative = 0; // 标记是否为负数
	...
	FraNum = (uint32_t)((Number - IntNum) * pow(10, FraLength) + 0.5); // 四舍五入
	...
	X += FontSize / 2; // 移动光标位置以显示后续数字

	// 显示整数部分
	OLED_ShowNumber(X, Y, IntNum, IntLength, FontSize);
	X += IntLength * FontSize / 2; // 根据整数长度调整X位置

	// 显示小数点
	OLED_ShowChar(X, Y, '.', FontSize, 1);
	X += FontSize / 2; // 调整X位置以显示小数部分

	// 显示小数部分
	OLED_ShowNumber(X, Y, FraNum, FraLength, FontSize);
}
