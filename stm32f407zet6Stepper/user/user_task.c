...
uint8_t main_task(void)
{
    ...
    base_run_distance_fix(61, run_speed, 0); // 去往二维码区域
    osDelay(200);
    ...
    base_run_distance_fix(79, run_speed, 0); // 去往原料区
    osDelay(1000);
    ...
    base_run_distance_fix(-35, run_speed, 0); // 去往粗加工区
    osDelay(200);
    /*
    base_rotation_world(-90, rot_speed);
...
*/
    ...
    base_run_distance_fix(-75, run_speed, -180); // 粗加工区到暂存区1
    osDelay(200);
    base_rotation_world(-90, rot_speed);
    osDelay(200);
    base_run_distance_fix(-78, run_speed, -90); // 粗加工区到暂存区2
    osDelay(200);
    ...
    base_run_distance_fix(-85, run_speed, -90); // 暂存区到原料区1
    osDelay(200);
    base_rotation_world(0, rot_speed);
    osDelay(200);
    base_run_distance_fix(-42, run_speed, 0);            // 暂存区到原料区2
    ...
    base_run_distance_fix(-35, run_speed, 0); // 去往粗加工区2
    osDelay(1000);
    base_rotation_world(0, rot_speed);
    ...
    base_run_distance_fix(-78, run_speed, -180);
    osDelay(200);
    base_rotation_world(-90, rot_speed);
    osDelay(200);
    base_run_distance_fix(-78, run_speed, -90);

    // TemporaryStorageArea_Task(); // 暂存区任务
    osDelay(1000);

    base_run_distance_fix(-90, run_speed, -90); // 去往原料区2
    osDelay(200);
    base_rotation_world(0, rot_speed);
    osDelay(200);
    base_run_distance_fix(-166, run_speed, 0);
